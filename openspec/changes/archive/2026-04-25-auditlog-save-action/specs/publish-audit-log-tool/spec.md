...
### Requirement: Agent writes directly to audit_logs
**Reason**: Direct persistence bypasses human review. The agent's classifications are judgment, not fact. A human must review and approve before the record becomes official.
**Migration**: `publish_audit_log` targets `draft_audit_logs` via internal endpoint.

### Requirement: Agent holds admin credentials
**Reason**: Giving an LLM admin access to the compliance evidence store is a DB grooming vector. Network-level isolation replaces credential-based trust.
**Migration**: Internal endpoint with NetworkPolicy restriction replaces API token auth.
