...
export async function listRepositories(registryUrl: string): Promise<Repository[]> {
  const res = await apiFetch(`${REGISTRY_API}/repositories?registry=${encodeURIComponent(registryUrl)}`);
  if (!res.ok) throw new Error(`Failed to list repos: ${res.status}`);
  return res.json();
}

export async function listTags(reference: string): Promise<Tag[]> {
  const res = await apiFetch(`${REGISTRY_API}/tags?ref=${encodeURIComponent(reference)}`);
...
export async function fetchManifest(reference: string): Promise<Manifest> {
  const res = await apiFetch(`${REGISTRY_API}/manifest?ref=${encodeURIComponent(reference)}`);
  if (!res.ok) throw new Error(`Failed to fetch manifest: ${res.status}`);
  return res.json();
}

export async function fetchLayer(reference: string): Promise<string> {
  const res = await apiFetch(`${REGISTRY_API}/layer?ref=${encodeURIComponent(reference)}`);
