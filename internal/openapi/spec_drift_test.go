...
func buildRouter(t *testing.T) *echo.Echo {
	...
	authHandler.RegisterChatHistory(apiGroup, auth.NewMemoryChatStore())

	registry.Register(apiGroup, registry.Options{InsecureRegistries: []string{"stub.local"}})
	publish.Register(apiGroup, publish.Options{})
	agents.RegisterDirectory(apiGroup, nil)
	...
	// Gemara proxy routes (replicate registerGemaraProxy from main.go)
	apiGroup.POST("/validate", func(c echo.Context) error { return nil })
	apiGroup.POST("/migrate", func(c echo.Context) error { return nil })
...
// wildcardSpecRoutes returns spec paths that use a wildcard parameter,
// mapped to the prefix for matching against Echo's /api/a2a/* pattern.
// OpenAPI models these as /api/a2a/{agentName} with individual methods,
// while Echo registers a single Any("/a2a/*") covering all methods.
var wildcardMappings = map[string]string{
	"/api/a2a/{agentName}": "/api/a2a/*",
}
