...
|:--|:--|:--|
| **ComplyTime Platform** | Headless API: OAuth, REST `/api/*`, A2A proxy, optional MCP proxy, evidence/certifier pipeline, notifications | Go gateway (`cmd/gateway`), ClickHouse (analytics store), optional PostgreSQL for auth/users where configured, optional NATS for events |
