...
type RegistryState = { phase: "input" } | { phase: "repos"; registry: string; repos: Repository[] } | { phase: "tags"; registry: string; repo: string; tags: Tag[] } | { phase: "manifest"; registry: string; repo: string; tag: string; manifest: Manifest } | { phase: "layer"; content: string; mediaType: string };
...
export function RegistryBrowser() {
  ...
  async function handleSelectRepo(repo: string) {
    if (state.phase !== "repos") return; setLoading(true); setError("");
    try { const tags = await listTags(`${state.registry}/${repo}`); setState({ phase: "tags", registry: state.registry, repo, tags }); }
    catch (e: unknown) { setError((e as Error).message); } finally { setLoading(false); }
  }
  async function handleSelectTag(tag: string) {
    if (state.phase !== "tags") return; setLoading(true); setError("");
    try { const manifest = await fetchManifest(`${state.registry}/${state.repo}:${tag}`); setState({ phase: "manifest", registry: state.registry, repo: state.repo, tag, manifest }); }
    catch (e: unknown) { setError((e as Error).message); } finally { setLoading(false); }
  }
  async function handleInspectLayer(digest: string, mediaType: string) {
    if (state.phase !== "manifest") return; setLoading(true); setError("");
    try { const content = await fetchLayer(`${state.registry}/${state.repo}@${digest}`); setState({ phase: "layer", content, mediaType }); }
    catch (e: unknown) { setError((e as Error).message); } finally { setLoading(false); }
  }
  ...
  return (
    <div class="registry-browser">
      ...
      {state.phase === "layer" && (<div class="registry-layer-view"><div class="manifest-media-type">{state.mediaType}</div><pre class="registry-layer-content">{state.content}</pre></div>)}
    </div>
  );
}
