...
func PopulateCatalogsFromRegistry(ctx context.Context, cs CatalogStore, ctrlS ControlStore, threatS ThreatStore, registryAddr string) error {
	if registryAddr == "" {
		return nil
	}
	...
	for _, sc := range defaultSeedCatalogs {
		...
		parseCatalogStructuredRows(ctx, sc.Type, content, catalogID, "", ctrlS, threatS)
		imported++
