...
If either is missing AND not already provided in the conversation history, ask once and stop. If studio-mcp is unavailable or resource reads fail, report the error and halt.
...
## Platform data access (studio-mcp)

Use MCP **resources** (not SQL):

| Need | URI / pattern |
|:-----|:--------------|
| Policy list (metadata) | `studio://policies` |
| Full policy by id | `studio://policies/{policy_id}` |
| Evidence rows | `studio://evidence?policy_id=…&limit=…&offset=…` |
| Posture aggregates | `studio://posture?policy_id=…` |
| Audit logs | `studio://audit-logs?policy_id=…` |
| Mapping documents | `studio://mappings` (optional `source_catalog=…`) |
| Catalog index | `studio://catalogs` |
| Threats | `studio://threats?catalog_id=…` |
| Risks | `studio://risks?catalog_id=…` |

Paginate evidence with `limit` / `offset` when result sets are large. Filter JSON client-side by audit window (`collected_at` or equivalent fields in each record) after loading evidence for the policy.
...
1. **Load Policy** — read `studio://policies` or `studio://policies/{policy_id}` and locate the policy YAML `content`. Parse it to extract `adherence.assessment-plans[]`. If no assessment plans exist, report "Policy has no assessment plans defined" and halt.
2. **Discover targets** — read `studio://evidence?policy_id=…` (paginate as needed) and collect distinct `target_id` / `target_name` within the audit window.
3. **Check each plan per target** — for each assessment plan, for each target:
   - Filter evidence from resource payloads by control/plan relevance within the frequency-derived window. Use `requirement_id` or `plan_id` only when present and non-empty; these fields are often null.
...
1. **Load Policy** — read `studio://policies/{policy_id}` (or resolve from `studio://policies`). Parse YAML `content` for imported catalog references and criteria set.
2. **Load MappingDocuments** — read `studio://mappings` and filter rows by `policy_id` in JSON (there is no `policy_id` query parameter on this resource). If none exist, skip cross-framework analysis and state this.
3. **Discover targets** — from `studio://evidence?policy_id=…`, derive distinct targets within the audit window. Present the inventory.
4. **Assemble evidence per target** — for each target, filter evidence from resource payloads for that target and policy criteria. Present a factual evidence summary table per target: Criteria ID, Evidence Count, Latest Date, Source Engine, Eval Result. No classifications — just data.
...
6. **Cross-framework coverage** — only when step 2 returned mappings. Combine classifications with mapping entries from the mappings resource payload (`mapping_entries` / equivalent in JSON).
...
8. **Publish as Draft** — after validation succeeds, call `save_draft_audit_log` with:
   - `yaml`: the validated YAML string (same content you validated)
   - `policy_id`: from the policy record — do NOT omit
   - `agent_reasoning`: JSON object mapping each result id to your classification reasoning (e.g. `{"bp-1-result": "Classified as Strength because 3/3 evidence records show Passed with correct engine provenance within the 30-day cadence window."}`)
   - Optionally `model` and `prompt_version` if the runtime exposes them.

   Do NOT put `agent-reasoning` in the YAML — it is not in the Gemara schema. Pass reasoning through `agent_reasoning`. Tell the user: "Draft AuditLog saved for review. A reviewer must promote it to the official audit history."
...
Infer field names and shapes from the JSON returned by each `studio://` resource. Use `studio://catalogs`, `studio://threats`, and `studio://risks` when you need catalog metadata. The studio-audit skill supplements classification rules; platform facts come from MCP resources above.

## Constraints

- Load evidence via `studio://evidence` before classifying. Never fabricate evidence.
